...
function youtube_loader( $html ) {
	$html = preg_replace_callback(
		...
		function ( $m ) {
				return '<div class="youtube__loader" title="' . $m[1] . '" data-ytid="' . $m[4] . '"><img class="youtube__loader--img" data-src="https://i.ytimg.com/vi/' . $m[4] . '/hqdefault.jpg" style="opacity: 0; transition: opacity .5s" alt="' . $m[1] . '"></div>';
		},
		$html
	);
...
function youtube_loader2( $html ) {
	$html = preg_replace_callback(
		...
		function ( $m ) {
				return '<div class="youtube__loader" title="' . $m[5] . '" data-ytid="' . $m[3] . '"><img class="youtube__loader--img" data-src="https://i.ytimg.com/vi/' . $m[3] . '/hqdefault.jpg" style="opacity: 0; transition: opacity .5s" alt="' . $m[5] . '"></div>';
		},
		$html
	);
...
function elementor_youtube_loader( $html ) {
	$html = preg_replace_callback(
		...
		function ( $m ) {
				return '<div' . $m[1] . ' elementor-widget-video">' . $m[7] . $m[9] . '<div class="youtube__loader youtube__loader--elementor" title="" data-ytid="' . $m[4] . '" data-width="" data-height=""><img class="youtube__loader--img" data-src="https://i.ytimg.com/vi/' . $m[4] . '/hqdefault.jpg" style="opacity: 0; transition: opacity .5s" alt=""></div>';
		},
		$html
	);
